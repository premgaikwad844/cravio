<?php
// snacks.php
$snacks = include 'snacks_data.php';
?>
...
<body>
    <nav class="navbar" style="background: #fff; box-shadow: 0 2px 8px rgba(0,0,0,0.04); padding: 18px 0;">
        ...
        <ul class="navbar-links" style="display: flex; list-style: none; margin: 0; padding: 0; align-items: center;">
            ...
            <li><a href="contact.php" style="color: #23272b; text-decoration: none; font-weight: 600; font-size: 1.05rem;">Contact Us <span class="arrow">&#8250;</span></a></li>
        </ul>
    ...
    <h1 class="fastfood-header">Popular Snacks</h1>
    <div class="fastfood-grid">
        <?php foreach ($snacks as $snack): ?>
            <div class="fastfood-card" style="display:grid;grid-template-rows:160px 1fr auto auto;min-height:370px;">
                <img class="fastfood-img" src="<?php echo htmlspecialchars($snack['image']); ?>" alt="<?php echo htmlspecialchars($snack['name']); ?>">
                <div class="fastfood-title" style="font-size:1.18rem;font-weight:700;color:#23272b;margin:16px 0 8px 0;"><?php echo htmlspecialchars($snack['name']); ?></div>
                <div class="fastfood-desc" style="font-size:0.98rem;color:#444;margin-bottom:24px;"><?php echo htmlspecialchars($snack['description']); ?></div>
                <a href="dishdetails2.php?dish=<?php echo urlencode($snack['name']); ?>" class="fastfood-view-btn" style="margin-top:auto;margin-bottom:0;align-self:center;display:inline-block;padding:7px 18px;background:#97c933;color:#fff;border-radius:6px;text-decoration:none;font-weight:600;transition:background 0.2s;">View</a>
            </div>
        <?php endforeach; ?>
    </div>
